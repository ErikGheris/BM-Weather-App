package com.example.bmweather.FetchingData
...
class FetchWeatherDataLocation {
    companion object FetchWeatherDataLocation {
        val TAG: String?=
            com.example.bmweather.FetchWeatherData.FetchWeatherData::class.java.simpleName
        private val apiRequest: WeatherService  = RetrofitRequest.getRetrofitInstance().create(WeatherService::class.java)


        fun getCurrentLocationWeatherReport(app_id: String,lat: String, lon: String,lang: String,units: String, mainActivity: MainActivity) {


            Log.d(TAG, "onResponse response:: $app_id  $lat $lon $lang $units")
            apiRequest.getCurrentLocationWeatherData(lat,lon,units,lang,app_id)
